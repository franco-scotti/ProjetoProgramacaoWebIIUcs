...
if ($fornecedorId !== null) $stmtCount->bindValue(':fid', $fornecedorId, PDO::PARAM_INT);
$stmtCount->execute();
...
if ($fornecedorId !== null) $stmt->bindValue(':fid', $fornecedorId, PDO::PARAM_INT);
...
if ($paginaAtual === 1) {
    $stmtF = $pdo->prepare(
        "SELECT pr.foto, pr.nome
         FROM item_pedido ip
         LEFT JOIN produto pr ON pr.id = ip.produto_id
         WHERE ip.pedido_id = :pid
           AND pr.foto IS NOT NULL" . $filtroFornFotos
    );
    $stmtF->bindValue(':pid', $pedidoId, PDO::PARAM_INT);
    if ($fornecedorId !== null) $stmtF->bindValue(':fid', $fornecedorId, PDO::PARAM_INT);
    $stmtF->execute();
